# 🍽️ Preferințe & restricții — Adi

> Lista de alimente e comună, în [`comun/1_ingrediente.md`](../comun/1_ingrediente.md).
> Aici scriem **doar ce iese din tipar la Adi** — restul se înțelege că e ok.

**Legenda:** ⭐ favorit · ❌ nu-i place · ⛔ nu poate (alergie/medical) · 🟡 tolerat, nu des · ❔ de testat

---

## ⛔ Nu poate (alergii, intoleranțe, medical)

**Niciuna.** Fără alergii, fără intoleranțe, fără condiții medicale sau medicație care să restrângă alimentația.

## ⭐ Favorite

| Aliment / grupă | Notă |
|-----------------|------|
| **Ridichi** | Emei nu-i plac → se pun **separat, la porția lui** (salate, garnituri crude), nu gătite în rețeta comună |

## ❌ Nu-i plac

Aceleași ca la Ema:

| Aliment / grupă | Notă |
|-----------------|------|
| **Curcan, fructe de mare, alți pești** | Nu se gătesc. Carne: **pui, porc (mușchi, cotlet; ceafă rar), vită**. Pește: **chefal, păstrăv, doradă, ton**; somon mai rar |

---

## Reguli proprii de frecvență

Aceleași ca la Ema — simplifică gătitul la comun:

| Regulă | Detaliu |
|--------|---------|
| **Pește 2×/săptămână** | Fix. Prioritar **pește gras** (somon, macrou, sardine, hering, păstrăv) pentru omega-3 |
| **Ouă: 4 la două zile** | O zi da / o zi pauză, sincronizat cu Ema (care mănâncă 3). Preferința lui — îi place mai mult, plus proteina extra ajută la recompoziție. În zilele fără ouă → altă sursă de proteină |
| **Fără lactate cu carne/pește, fără ouă cu carne** | În aceeași masă. Lactatele merg la micul dejun (cu ouă e ok) și la gustări; la felurile cu carne, sosurile sunt fără iaurt/brânză |

## Preferințe culinare

| Aspect | Valoare |
|--------|---------|
| Nr. mese/zi | **3 mese + 1 gustare** *(ca Ema, presupus — de confirmat)* |
| Orele meselor | ⟨de stabilit⟩ |
| Gustarea | de preferat **după sală** (proteină + carbohidrați) |
| Timp de gătit realist | ⟨de stabilit⟩ |
| Metode preferate (cuptor, tigaie, air fryer, slow cook) | ❔ |
| Nivel de condimentare | ❔ |
| Toleranță la repetiție | ❔ |
| Gătește la comun cu | **Ema** |

---

## Ce a zis despre rețete

De aici se completează coloana „Adi" din [`comun/2_retete.md`](../comun/2_retete.md).

| Rețetă | Verdict | Notă |
|--------|:-------:|------|
| *(gol — aștept prima rețetă)* | | |
