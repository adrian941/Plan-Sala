...
**Regulă permanentă — caietul îngust, doar cu rețete (`index.html?caiet=retete`).** Pe lângă caietul mare de mai sus există un al doilea caiet, cerut ca **„PDF doar cu rețete, cât o treime de A4"**: aceleași cartonașe, aceeași cerneală, dar foaia e **99 × 210 mm** — exact o treime dintr-un A4 culcat, adică **lățimea unei coloane de rețete din caietul mare** (trei foi înguste una lângă alta = o foaie mare). Pe telefon foaia iese cât ecranul, deci se citește **derulând, fără zoom și fără rotit**.
- Se deschide adăugând `?caiet=retete` la adresa site-ului. Pagina nu mai desenează nimic din site (meniu, butoane, cipuri) — doar cartonașele, unul sub altul; pe ecran se văd ca foi albe, exact cum vor ieși în PDF.
- **Rețetele sunt despărțite pe tipuri**, cu titlu de grup înainte de fiecare (Mic dejun · Gustări · Feluri principale), exact ca pe pagina „Rețete" a site-ului. Ordinea vine deja așa din bază, deci titlul se pune acolo unde se schimbă grupul, și e **lipit de primul card al grupului** (cutia `.rgr`, măsurată împreună cu el) — altfel ar putea rămâne singur la baza unei coloane, cu rețeta lui pe foaia următoare. Titlurile sunt **doar în caietul îngust**: caietul mare rămâne cum e.
- **Ce se schimbă față de caietul mare, tot din lățime:** o singură coloană pe foaie (`--ret-col: 1`) și fără micșorare (`--fit: 1`); **ingredientele trec pe două coloane, pe toată lățimea**, iar pașii vin dedesubt — cu grila veche (29 mm ingrediente | pași) pașii ar fi rămas pe ~55 mm și sub lista de ingrediente ar fi rămas o jumătate de foaie albă. Restul (titlu, linia de kcal/macro/vitamine, ordinea rețetelor, regula „un card nu se rupe niciodată între două foi") rămâne neatins.
- Geometria stă tot în CSS (`html.caiet-retete` din `_site/style.css`), fiindcă **cutia de măsurat `.rmas` trebuie să vadă exact aceleași reguli** — altfel împărțirea pe foi iese greșită. Singura excepție: `@page` (99 × 210 mm, margine 4 mm) e scris din `app.js`, fiindcă o regulă de document nu se poate pune pe o clasă; de aceea și blocul „orientation: portrait" din CSS e oprit pentru caietul îngust (el e portret **dinadins**, n-are ce roti).
- **Exportul din chat**: la fel ca mai sus, dar cu `?caiet=retete` în adresă și **fără `landscape: true`** — `page.pdf({ printBackground: true, preferCSSPageSize: true })`. Verifică înainte să trimiți: paginile trebuie să iasă **281 × 595 pt** (99 × 210 mm, deci *mai înalte decât late*, invers față de caietul mare) și niciun card tăiat (`scrollHeight` vs. `clientHeight` pe `.rp`). Azi: **19 rețete pe 10 foi**.
...
- **Conținutul se actualizează singur la fiecare push.** Service worker-ul ia **întâi din rețea** și abia dacă nu e net cade pe copia locală — deci cine deschide aplicația cu net vede imediat ultima versiune, instalată sau nu. Nu trebuie făcut nimic special.
- **Când schimbi ceva în site, crești totuși `VERSIUNE` din `sw.js`** (`plan-sala-v1` → `v2`). Nu pentru prospețime (vezi mai sus), ci ca să se șteargă copia veche de offline, cu tot cu fișierele care nu mai există.
- **Iconița și numele de pe ecranul principal fac excepție:** ele se fixează la instalare. Pe Android, Chrome recitește manifestul din când în când și schimbă iconița după o zi-două; **pe iPhone nu se schimbă deloc** — aplicația trebuie ștearsă și adăugată din nou. La fel și `display: fullscreen`. Deci o iconiță nouă nu apare instant la cine are deja aplicația pusă.
