# 🍽️ Preferințe & restricții — Ema

> Lista de alimente e comună, în [`comun/1_ingrediente.md`](../comun/1_ingrediente.md).
> Aici scriem **doar ce iese din tipar la Ema** — ce nu apare mai jos se înțelege că e ok.

**Legenda:** ⭐ favorit · ❌ nu-i place · ⛔ nu poate (alergie/medical) · 🟡 tolerat, nu des · ❔ de testat

---

## ⛔ Nu poate (alergii, intoleranțe, medical)

**Niciuna.** Fără alergii, fără intoleranțe, fără condiții medicale sau medicație care să restrângă alimentația.

## ⭐ Favorite

| Aliment / grupă | Notă |
|-----------------|------|
| **Lactate** (iaurt grec, brânză de vaci, telemea, cașcaval, kefir…) | **Preferă lactatele peste carne** — sursa ei principală de proteină |
| Vită slabă | Carnea preferată |
| Porc slab | Carnea preferată |
| Pui | Carnea preferată |
| Legume, cât mai variate | Vrea **diversitate**, nu mereu aceleași |

## ❌ Nu-i plac

*(Niciuna semnalată încă. Se completează pe măsură ce apar rețete.)*

| Aliment / grupă | Notă |
|-----------------|------|
| — | |

## 🟡 Tolerate / ❔ de testat

| Aliment / grupă | Verdict | Notă |
|-----------------|:-------:|------|
| Tofu, edamame | ❔ | Netestate |

---

## Reguli proprii de frecvență

| Regulă | Detaliu |
|--------|---------|
| **Pește 2×/săptămână** | Fix. Prioritar **pește gras** (somon, macrou, sardine, hering, păstrăv) pentru omega-3 |
| **Ouă: 3 la două zile** | O zi da / o zi pauză. În zilele fără ouă → altă sursă de proteină |
| Legume | Se rotesc grupele, nu se repetă aceleași zile la rând |

## Preferințe culinare

| Aspect | Valoare |
|--------|---------|
| Nr. mese/zi | **3 mese + 1 gustare** |
| Orele meselor | ⟨de stabilit⟩ |
| Gustarea | de preferat **~20 min după sală** |
| Timp de gătit realist | ⟨de stabilit⟩ |
| Metode preferate (cuptor, tigaie, air fryer, slow cook) | ❔ |
| Nivel de condimentare | ❔ |
| Bucătării preferate | ❔ |
| Toleranță la repetiție | scăzută la legume — vrea variație |
| Gătește la comun cu | **Adi** |

---

## Ce a zis despre rețete

De aici se completează coloana „Ema" din [`comun/2_retete.md`](../comun/2_retete.md).

| Rețetă | Verdict | Notă |
|--------|:-------:|------|
| *(gol — aștept prima rețetă)* | | |
